...
echo -e "${COLOR}Vulkan drivers version...${OUT}"; vulkaninfo 2>&1| grep -iEm  2 'GPU ID = 0|VULKAN INSTANCE VERSION' | grep -v WARNING
echo -e "${COLOR}\nReboot your PC after 30 minutes, cause after the installation, the drivers will start the compilation process.${OUT}"
